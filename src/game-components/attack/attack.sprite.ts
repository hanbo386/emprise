...
export class Attack {
  ...
  constructor (
    ...
    this.shortName = this.id.split('-')[0];
    ...
    for (let id in this._heroShapes) {
      if (id != this.sourceHeroId) {
        ...
        if (this.checkHitHero(this._heroShapes[id])) {
          console.log(`${getShortName(this.sourceHeroId)}'s attack ${this.shortName} hit hero ${getShortName(id)}.`);
          console.log('Going to destroy this attack...');
          this.cleanUpAttack();
          break;
        }
